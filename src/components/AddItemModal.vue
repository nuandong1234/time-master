...
const props = defineProps<{ modelValue: boolean; hideRepeat?: boolean; hideContinuousCreate?: boolean }>()
const emit = defineEmits<{ "update:modelValue": [value: boolean]; create: [data: { name: string; description: string; startDate: string; endDate: string; priority: string; repeatType: string }] }>()
...
function resetForm() {
  form.value = { name: "", description: "", startDate: "", endDate: "", priority: "", repeatType: "none" }
  closeCal()
}
  ...
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center" @keydown="handleKeydown">
    ...
    <div class="relative bg-card border border-border rounded-lg shadow-lg w-[520px] p-5">
      ...
      <div class="flex flex-col gap-3">
        ...
        <div v-if="showCal" class="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div class="relative bg-card border border-border rounded-lg shadow-lg p-4 w-[280px] select-none" @wheel.prevent="handleCalWheel">
            ...
            <div v-if="!hideRepeat" class="mt-3 pt-3 border-t border-border flex items-center gap-2 text-xs">
      ...
      <div class="flex items-center justify-between mt-5">
        <label v-if="!hideContinuousCreate" class="flex items-center gap-1.5 cursor-pointer text-sm text-muted-foreground">
          <input type="checkbox" v-model="continuousCreate" class="accent-primary" />
          连续创建
        </label>
        <div v-else></div>
        <div class="flex gap-2">
          ...
          <button @click="handleCreate" class="text-sm px-4 py-1.5 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">创建</button>
        </div>
      </div>
    </div>
  </div>
