...
import { completeItem, updateItem, deleteItem, clearDoneItems } from "@/lib/workflow-item-sync"

const { addItem, uncompleteItem, loadItems, addItemComment } = useItems()
...
const { todoItems, syncedItems, doneItems, items } = useItems()
...
async function handlePanelComment(data: { itemId: number; content: string; images?: string[] }) {
  await addItemComment(data.itemId, data.content, data.images)
}
</script>
