...
import { completeItem, updateItem, deleteItem } from "@/lib/workflow-item-sync"
...
<template>
  <div class="h-full p-px grid grid-cols-3 grid-rows-[6fr_4fr] gap-px select-none">
    ...
    >
      <h3 class="text-lg font-semibold text-card-foreground">番茄计时</h3>
    ...
    >
      ...
      <div class="flex-1 grid grid-cols-2 grid-rows-2 gap-[10px]">
        ...
        <div class="bg-muted rounded-md flex flex-col items-center justify-center gap-0.5">
          ...
          <span class="text-xl font-semibold text-card-foreground">{{ (s => s < 60 ? s + '秒' : s < 3600 ? Math.floor(s / 60) + '分' + (s % 60 ? s % 60 + '秒' : '') : Math.floor(s / 3600) + '小时' + (Math.floor(s % 3600 / 60) ? Math.floor(s % 3600 / 60) + '分' : '') + (s % 60 ? s % 60 + '秒' : ''))(Math.round(totalFocusSeconds)) }}</span>
        </div>
      </div>
    </section>
