...
const DATA_FILE = "data/workflow.json"
...
export async function initWorkflow(force = false) {
  if (!state.loaded || force) {
    const loaded = await loadWorkflow()
    if (!loaded) {
      // 文件不存在，保持空数据，等用户第一次操作时自动创建文件
    }
    // 确保"已完成"目录存在
    ensureDoneCategory()
    // 确保至少有一个普通目录
...
export async function removeCategory(id: number) {
  ...
  for (const pid of [...cat.projectIds]) {
    ...
    if (pi !== -1) state.projects.splice(pi, 1)
  }
...
export async function completeNodeAndAdvance(stepIdx: number, nodeIdx: number) {
  ...
  // 节点 done → 同步完成关联事项，同时同步日期
  if (node.itemId) {
    const { markItemAsDone, updateItem } = await import("./items")
    await markItemAsDone(node.itemId)
    await updateItem(node.itemId, {
      name: node.name,
      description: node.description || '',
...
export async function updateNodeDetail(
  ...
  // 节点编辑 → 同步回关联事项
  if (node.itemId && (data.name !== undefined || data.description !== undefined || data.startDate !== undefined || data.endDate !== undefined)) {
    try {
