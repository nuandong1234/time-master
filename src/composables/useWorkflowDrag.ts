...
export function useWorkflowDrag(
  ...
  function canDropAtNode(dragNode: WorkflowNode, targetNode: WorkflowNode): boolean {
    if (dragNode.status === "wait" && targetNode.status !== "wait") return false
    if (dragNode.status === "active" && targetNode.status === "done") return false
    return true
  }

  function canDropAtStepEnd(dragNode: WorkflowNode, toStep: WorkflowStep): boolean {
    if (dragNode.status === "wait") {
      const firstNonWaitIdx = toStep.nodes.findIndex(n => n.status !== "wait")
      if (firstNonWaitIdx >= 0 && toStep.nodes.length <= firstNonWaitIdx) return false
    }
    return true
  }
  ...
  function executeDragMove(
    ...
    if (to.nodeIdx === -1) {
      ...
      if (!targetNode || !canDropAtNode(dragNode, targetNode)) {
        showToast('当前节点无法移动到此处')
        resetDragState()
        return
      }
